...
echo "$(date +'%m-%d-%y %H:%M:%S') Starting Sphinx Build" >> $log
...
for html in $htmlfiles; do
    ...
    if [ "$html" != "genindex.html" ] && [ "$html" != "search.html" ]; then
        ...
        if [ -e "$rst" ]; then
            ...
            set_list=`cat $html | grep SEO_META_ | sed -e 's/="/ /g' | sed -e 's/"//g' | sed -e 's/>//g' | awk '{print $NF}'`
            for setval in $set_list; do
                rstval=`cat $rst | grep $setval | sed -e "s/$setval=\"/ /g" | sed "s/^[ \t]*//" | sed -e 's/"//g' | sed -e 's/>//g'`
        ...
        sed -i "s|SEO_META_URL|http://jaypjohnson.com/|g" $html
        sed -i "s|SEO_META_TITLE|Docker + Django with Slack Debugging and Sphinx Doumentation|g" $html
        sed -i "s|SEO_META_DESC|A Docker container running a django webserver that is integrated with Slack debugging and Sphinx Bootstrap for Documentation. It also shares a static volume for combining with nginx or apache.|g" $html
        sed -i "s|SEO_META_KEYWORD|software, docker, django, slack, sphinx|g" $html
        sed -i "s|SEO_META_CREATOR|@jayphjohnson|g" $html
        sed -i "s|SEO_META_TYPE|website|g" $html
        sed -i "s|SEO_META_ALSO|http://jaypjohnson.com/|g" $html
        sed -i "s|SEO_META_IMAGE|/static/_images/image_logo.png|g" $html
        sed -i "s|SEO_META_AUTHOR|Jay Johnson|g" $html
    fi
